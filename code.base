Var
  ...
  begin
    ...
    if (centX <= krdnresimMax_X) and (centX >= krdnresimMin_X) and   
       (centY <= krdnresimMax_Y) and (centY >= krdnresimMin_Y) then
  ...
  begin  
    ...
    Begin
      ...
      if Clomosy.PlatformIsMobile Then DeviceMotionSensor.Active := True; // Bu jireskop özeliğini açıyor.
    end else if Clomosy.PlatformIsMobile Then DeviceMotionSensor.Active := False; // Bu jireskop özeliğini kapatıyor.
  end;
  ...
  begin
    ...
    Begin
      ...
      begin
        ...
        If Clomosy.PlatformIsMobile Then DeviceMotionSensor.Active := False; //Oyunu durdurur.
...
Begin
  ...
  clComponent.SetupComponent(GoBackBtn,'{"Align" : "None","Height":50,"Width":50,"ImgUrl":"https://img.icons8.com/flat-round/64/circled-left.png"}');
